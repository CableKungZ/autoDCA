<template>
  <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" @click.self="$emit('close')">
    <div class="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
      ...
      <div class="px-6 py-5 flex flex-col gap-5">
        ...
        <!-- Percent: slider -->
        <div v-if="orderType === 'percent'">
          <p class="text-xs text-gray-400 mb-2 font-medium uppercase tracking-wide">Percentage to sell</p>
          ...
          <p class="text-xs text-gray-500 mt-2">
            Selling ≈ {{ fmt(holdings * sellPercent / 100, 8) }} {{ baseToken }}
            of {{ fmt(holdings, 8) }} {{ baseToken }}
          </p>
        </div>

        <!-- Limit price -->
        <div v-if="orderType === 'limit'">
          <p class="text-xs text-gray-400 mb-2 font-medium uppercase tracking-wide">Limit price ({{ quoteToken }})</p>
          <div class="flex gap-2 items-center">
            <input v-model.number="limitPrice" type="number" min="0" step="any" :placeholder="`Current price: ${fmt(currentPrice)}`"
              class="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-lg text-gray-100 font-mono focus:outline-none focus:border-red-500" />
        ...
        <div class="bg-gray-800/60 border border-gray-700 rounded-xl px-4 py-3 text-sm flex flex-col gap-1.5">
          <div class="flex justify-between text-gray-400">
            <span>Type</span><span class="text-white">{{ orderType.toUpperCase() }}</span>
          </div>
          <div class="flex justify-between text-gray-400">
        ...
        <div class="flex gap-3">
          ...
          <button type="button" @click="confirm" :disabled="!canSubmit || loading"
            ...
            {{ loading ? 'Submitting...' : `Sell ${baseToken}` }}
          </button>
        </div>
      </div>
    </div>
  ...
  <div v-if="showConfirm" class="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
    <div class="bg-gray-900 border border-red-700 rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl">
      ...
      <p class="font-semibold text-lg mb-1">Confirm Sell</p>
      <p class="text-gray-400 text-sm mb-2">{{ plan.symbol }} · {{ plan.exchange.toUpperCase() }}</p>
      <p class="text-red-300 text-sm mb-1">Sell <b>{{ fmt(sellQty, 8) }} {{ baseToken }}</b></p>
      <p class="text-green-400 text-sm mb-5">Est. receive <b>{{ fmt(estimatedReceive) }} {{ quoteToken }}</b></p>
      <div class="flex gap-3">
        <button @click="showConfirm = false" class="flex-1 bg-gray-800 hover:bg-gray-700 py-2.5 rounded-xl text-sm transition-colors">Cancel</button>
        <button @click="submit" class="flex-1 bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-xl text-sm font-medium transition-colors">Confirm Sell</button>
      </div>
    </div>
  </div>
</template>
...
const types = [
  { id: 'market' as OrderType,  icon: '⚡', label: 'Market',  desc: 'Market price' },
  { id: 'limit' as OrderType,   icon: '🎯', label: 'Limit',   desc: 'Set price' },
  { id: 'percent' as OrderType, icon: '%',  label: 'Percent', desc: '% of holdings' },
]
