  ...
  <div class="max-w-2xl">
    ...
    <div class="flex flex-col gap-4">
      <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 text-sm text-gray-300 flex flex-col gap-3">
        <p class="text-gray-400">ตั้งค่าผ่านไฟล์ <code class="text-indigo-300">.env</code> ที่ root ของโปรเจกต์ แล้ว restart:</p>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 text-sm flex flex-col gap-2">
        <p class="text-gray-400 font-medium mb-1">ตัวแปรที่ต้องตั้งค่า</p>
      ...
      <!-- Server IP -->
      <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 text-sm flex flex-col gap-2">
        <p class="text-gray-400 font-medium mb-1">Server Public IP (whitelist ใน Exchange)</p>
        <div class="flex items-center gap-3">
          <code class="text-green-300 text-base font-mono">{{ serverIp || 'กำลังโหลด...' }}</code>
        ...
        <p class="text-gray-600 text-xs">เพิ่ม IP นี้ใน Binance / Bitkub API Key whitelist</p>
      </div>

      <p class="text-yellow-400 text-sm bg-yellow-950 border border-yellow-900 rounded-lg px-4 py-3">
        ใช้ permission <b>read + trade only</b> ไม่ควรเปิด withdrawal permission
      </p>
    </div>
  </div>
</template>
...
const vars = [
  ...
  { key: 'TELEGRAM_BOT_TOKEN', desc: 'Telegram Bot Token จาก @BotFather' },
  { key: 'TELEGRAM_CHAT_ID', desc: 'Chat ID ของผู้รับแจ้งเตือน' },
  { key: 'FERNET_KEY', desc: 'Key สำหรับ encrypt API keys ใน DB' },
]
</script>
